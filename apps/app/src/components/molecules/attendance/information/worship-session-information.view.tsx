...
import { GRAY, MAIN, SIZE } from '@mokjang/constants';
...
const WorshipSessionInformationView = ({
  ...
  // ✅ 모든 정보가 비어 있는지 검사
  const isEmpty =
    !targetWorshipSession.title?.trim().length &&
    !targetWorshipSession.inCharge &&
    !targetWorshipSession.bibleTitle?.trim().length &&
    !targetWorshipSession.videoUrl?.trim().length &&
    !targetWorshipSession.description?.trim().length;
  ...
  return (
    <InformationContainer>
      ...
      {isEmpty ? (
        <EmptyList
          size={30}
          text={t_description('emptyWorshipSessionInformation')}
        />
      ) : (
        <MetaContainer>
          <RowContainer>
