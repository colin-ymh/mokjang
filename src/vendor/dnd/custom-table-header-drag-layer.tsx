...
export const CustomTableHeaderDragLayer: React.FC = () => {
  const { item, itemType, isDragging, currentOffset } = useDragLayer(
    (monitor) => ({
      item: monitor.getItem<{ title: string }>(),
      itemType: monitor.getItemType(),
      isDragging: monitor.isDragging(),
      currentOffset: monitor.getSourceClientOffset(),
    })
  );
  if (itemType !== DND_ITEM_TYPE.TABLE_HEADER || !isDragging || !currentOffset)
    return null;
...
export default CustomTableHeaderDragLayer;
