...
const UserFilteredItem = ({ item }: UserFilteredItemProps) => {
  const t = useI18n();
  ...
  const onClickCancel = () => {
    if ([USER.NAME].includes(item.title)) {
      dispatch(setUserFilter({ ...userFilter, [item.title]: BLANK }));
    }
    // else {
    //   dispatch(setUserFilter({ ...userFilter, [item.title]: [] }));
    // }
  };
...
export default UserFilteredItem;
